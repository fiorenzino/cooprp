...
public class RestStaticClient
{
            ...
            Map<String, Object> headerParams) throws Exception
   {
      try (WebTargetClosable webTargetClosable = new WebTargetClosable(targetHost, targetPath))
      {
         ...
         {
            String message = "[" + webTargetClosable.response.getStatus() + "] - " + webTargetClosable.response
   ...
   public static <T> T post(String targetHost, String path,
            Object entity,
            Class<T> t, Map<String, Object> queryParams,
            Map<String, Object> pathParams,
            Map<String, Object> headerParams) throws Exception
   {
      return post(targetHost + path, entity, t, queryParams, pathParams, headerParams);
   }
      ...
      try (WebTargetClosable webTargetClosable = new WebTargetClosable(targetHost, targetPath))
      {
         ...
         for (String key : multipartParams.keySet())
         {
            if (multipartParams.get(key) instanceof File)
            {
               ...
               multipartFormDataOutput.addFormData(fileKey, IOUtils.toByteArray(file.toURI()),
                        MediaType.APPLICATION_OCTET_STREAM_TYPE, key);
            }
            else if (multipartParams.get(key) instanceof InputStream)
            {
               String fileKey = streamCount == 0 ? "file" : "file" + (++streamCount);
               multipartFormDataOutput.addFormData(fileKey, (InputStream) multipartParams.get(key),
                        MediaType.APPLICATION_OCTET_STREAM_TYPE, key);
            }
            else if (multipartParams.get(key) instanceof byte[])
            {
               String fileKey = streamCount == 0 ? "file" : "file" + (++streamCount);
               multipartFormDataOutput.addFormData(fileKey, (byte[]) multipartParams.get(key),
                        MediaType.APPLICATION_OCTET_STREAM_TYPE, key);
            }
            else
            {
         ...
         {
            webTargetClosable.response = builder
                     .buildPost(Entity.entity(multipartFormDataOutput, MediaType.MULTIPART_FORM_DATA_TYPE)).invoke();
         }
         else if (method.equalsIgnoreCase(HttpMethod.PUT))
         {
            webTargetClosable.response = builder
                     .buildPut(Entity.entity(multipartFormDataOutput, MediaType.MULTIPART_FORM_DATA_TYPE)).invoke();
         }
         else
         {
